...
In this project, we explore the "neural" efficiency of two sensing modalities (vision and olfaction) for a hunting task. In both cases, an agent navigates a maze to locate a particular target. We then evaluate the efficiency of each modality by constraining the neural capacity of the agents. By comparing task performance across modalities with different neural capacity constraints, we can understand under which conditions vision outperforms olfaction (and vice versa).

In this optimization problem, our goal is to minimize the expected time taken to reach the target $T$, given a constraint on neural capacity, given by $max bb(E)_pi [T]: C(pi) <= kappa$. An alternative formuation would be $bb(E)_pi [T] - lambda C(pi$) where $lambda$ is a regularizing term.
